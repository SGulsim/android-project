...
object LocationHelper {
    suspend fun getCoordinatesFromCityName(context: Context, cityName: String, defaultCity: String = "Москва"): Pair<Double, Double>? {
            ...
            try {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                    ...
                    suspendCancellableCoroutine { continuation ->
                        geocoder.getFromLocationName(cityName, 1) { addresses ->
                            if (addresses.isNotEmpty()) {
                                ...
                                continuation.resume(getDefaultCoordinates(cityName, defaultCity))
                            }
                        }
                    }
                } else {
                    ...
                    if (addresses?.isNotEmpty() == true) {
                        ...
                        getDefaultCoordinates(cityName, defaultCity)
                    }
                }
            } catch (e: Exception) {
                getDefaultCoordinates(cityName, defaultCity)
    ...
    suspend fun getCurrentLocation(context: Context, defaultCity: String = "Москва"): Pair<Double, Double>? {
        return withContext(Dispatchers.IO) {
            try {
                ...
                } ?: getDefaultCoordinates(defaultCity, defaultCity)
            } catch (e: Exception) {
                getDefaultCoordinates(defaultCity, defaultCity)
            }
        }
    ...
    private fun getDefaultCoordinates(cityName: String, defaultCity: String): Pair<Double, Double>? {
        ...
        return defaultCities[cityName] ?: defaultCities[defaultCity]
    }
}
